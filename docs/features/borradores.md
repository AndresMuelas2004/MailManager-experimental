...
Un borrador conserva los destinatarios "Para", "Cc" y "Cco", el asunto y el cuerpo. El cuerpo es **texto plano**: el composer es un `<textarea>` simple y el correo se manda siempre como `text/plain`. Pegar HTML en el cuerpo guarda el marcado en crudo, no lo interpreta (la razón y las alternativas futuras están en [../limits/borradores.md](../limits/borradores.md)).

Un borrador heredado, creado antes del cambio a texto plano (o editado fuera de la app con cuerpo HTML), se sigue mostrando **tal cual** al abrirlo: su contenido se vuelca en el `<textarea>` sin interpretar, de modo que el usuario puede ver y editar las etiquetas en crudo. No se intenta convertir HTML a texto; es una limitación aceptada del MVP.
