...
- **Los diseños responsive**: las reglas que adaptan el correo a distintos anchos de pantalla sobreviven, así que las plantillas de newsletter modernas renderizan su versión de escritorio correctamente dentro del visor. Esto incluye maquetación moderna con flexbox y propiedades CSS lógicas, además de las tablas clásicas.
...
**Solo se proxean imágenes remotas `http(s)`.** Las imágenes **embebidas** (`cid:`, ya convertidas a `data:` — secciones 4 a 4.2) no cambian: siguen viajando dentro del propio HTML, sin red. Las **fuentes web** (`@font-face`) tampoco se proxean —el proxy solo sirve imágenes—, así que se cargan como antes.
