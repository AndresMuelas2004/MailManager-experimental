...
Mientras la ventana está abierta, la página de cuentas queda a la espera (el botón muestra "Conectando…"). Al completarse la autorización, aparece **de inmediato** la tarjeta de la cuenta nueva, ya usable — el usuario **no** queda bloqueado esperando una descarga. La app arranca en segundo plano la **descarga masiva** del histórico reciente de la cuenta, y la tarjeta muestra un contador en vivo *«Cargando… N correos»* que sube mientras la descarga avanza; cuando termina, el contador desaparece y quedan los mensajes recientes (o "Sin correos todavía" si la cuenta no tiene ninguno). Los **borradores** también se sincronizan al conectar, por una vía propia que el **servidor** dispara de forma fiable (con reintento si falla), independiente de la descarga de correo — ver [borradores.md](borradores.md) § 5.1. El detalle de esa carga inicial en segundo plano — cuánto tarda, cómo se reanuda, por qué reconectar no la relanza — vive en [sincronizacion.md](sincronizacion.md) § 3.5.
...
### 2.8 El límite de cuentas conectadas por usuario

Hay un tope de **cuántas cuentas de correo** puede tener conectadas un usuario **en total, sumando todas sus bandejas**. En cambio, el número de **bandejas** (buzones) que el usuario puede crear **no** está limitado: puede crear todas las bandejas unificadas que quiera; lo que está acotado es el total de cuentas de correo repartidas entre ellas. La cifra exacta del tope, que es **configurable por el administrador del despliegue**, está en [../limits/autenticacion-y-cuentas.md](../limits/autenticacion-y-cuentas.md).

Qué experimenta el usuario:

- En la página de "Cuentas conectadas" ve **en todo momento cuántas cuentas tiene y cuántas le quedan**, con un contador del tipo «conectadas / máximo» (por ejemplo, «12 / 15»).
- Cuando llega al máximo, el botón **"Añadir cuenta" se deshabilita** y aparece un aviso explicando que se ha alcanzado el tope. No hay forma de empezar el alta de una cuenta más desde la interfaz.
- Si por cualquier otra vía se intentara crear la cuenta que sobra, el **servidor la rechaza** con un mensaje claro (no se crea la cuenta ni queda a medias).

Dos matices deliberados:

- **Una cuenta cuyo acceso ha caducado sigue contando** para el tope hasta que el usuario la elimine. El límite mide "cuántas cuentas ocupan espacio en el servicio", no "cuántas están sanas": una cuenta a la que se le murió el token pero que el usuario aún no ha borrado sigue ocupando su hueco (para recuperarla, se reconecta, sección 2.7; para liberar el hueco, se elimina, sección 3.2).
- **Eliminar una cuenta libera un hueco** al instante: tras borrarla, el contador baja y el botón "Añadir cuenta" vuelve a habilitarse si estaba en el tope.

El porqué del límite es proteger el **almacenamiento** del servicio: cada cuenta conectada sincroniza hasta decenas de miles de cabeceras (más cuerpos y adjuntos cacheados bajo demanda), así que un tope por usuario acota el consumo. La semántica exacta del recuento (qué cuenta y qué no) y el código de error del rechazo están en [../limits/autenticacion-y-cuentas.md](../limits/autenticacion-y-cuentas.md).
