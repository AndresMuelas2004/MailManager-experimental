...
> **Nota — modo conversación.** Hoy las bandejas reales y la unificada se presentan **agrupadas por conversación**: las cifras de **tamaño de página y paginación** de este documento siguen valiendo (la página agrupa **hilos** en vez de mensajes, pero su tamaño es el mismo), mientras que el **total** pasa a contar hilos y la **selección/acciones masivas** desaparecen de esas bandejas (solo quedan en Favoritos). Las cifras propias de la agrupación (reglas de agregado, tamaño de lote del hilo por proveedor) están en **[conversaciones.md](conversaciones.md)**.
...
| Total devuelto (`total`) | **Conteo exacto del conjunto filtrado completo** en la copia local | Campo `total` del envoltorio `EmailPageOut` (backend) | Es el tamaño de TODO el conjunto (mismo `box`/`q`/`favorite`/cuentas), no el de la página; refleja solo lo **sincronizado**, nunca el buzón en vivo del proveedor. En modo conversación (`group_by_thread=true`) cuenta **hilos distintos**, no mensajes — ver [conversaciones.md](conversaciones.md). |
...
| Orden de resultados (por defecto) | `received_at` **descendente**, con desempate fijo por `(account_id, provider_message_id)` | Backend (misma consulta para bandeja real y ficticia) | El desempate hace el orden **total** y determinista; sin él, `OFFSET` produciría duplicados/saltos entre páginas. Es la garantía que hace segura la paginación por posición. En las bandejas reales el usuario puede **cambiar** orden (fecha / remitente / asunto, asc/desc) y aplicar chips de filtro — valores exactos en [ordenar-y-filtrar-listado.md](ordenar-y-filtrar-listado.md); el desempate se mantiene en todos los casos. |
...
| Tope de selección "seleccionar todo" | **50 correos** (los de la página actual) | Selección en la UI para acciones en bloque — **hoy solo en Favoritos** | La selección por fila desapareció de las bandejas en modo conversación; solo Favoritos la conserva. Donde existe, la casilla de cabecera marca como mucho la página visible (50 = tamaño de página); es un límite de **selección por página**, no de carga, y la selección **persiste entre páginas**. Detalle en [acciones-sobre-correos.md](acciones-sobre-correos.md) y [conversaciones.md](conversaciones.md). |
...
| Bandejas navegables | **5** (`ALL_MAIL`, `SENT`, `ARCHIVE`, `SPAM`, `TRASH`) | Parámetro `box` (obligatorio) | `ARCHIVE` es la bandeja "Archivados" (ver [acciones-sobre-correos.md](acciones-sobre-correos.md)). El estado interno "eliminado definitivamente" (`DELETED`) **no** es navegable y nunca aparece en una lista. |
...
- **El orden por defecto es fecha descendente, pero en las bandejas reales se puede reordenar** por fecha / remitente / asunto (asc o desc) y filtrar con chips — ver [ordenar-y-filtrar-listado.md](ordenar-y-filtrar-listado.md). Lo que **no** ofrece esa funcionalidad: ordenar por **tamaño** ni por **relevancia**, ni orden multi-criterio elegible. *Por qué:* el tamaño no se almacena en metadata y la relevancia está descartada también en la lupa; los topes y exclusiones exactos están en [ordenar-y-filtrar-listado.md](ordenar-y-filtrar-listado.md). En **Favoritos** y en las **bandejas ficticias** sigue sin haber reordenación interactiva (fuera del alcance de esa feature).
...
- **No se filtra por más de un `box` a la vez en una bandeja real.** Cada vista enseña exactamente una de las bandejas navegables; no hay una vista "todo junto incluyendo papelera y spam". *Por qué:* la clasificación es excluyente (un correo está en una sola bandeja) y mezclar papelera/spam con el resto confundiría. Las vistas multi-criterio son terreno de las bandejas ficticias ([bandejas-ficticias.md](bandejas-ficticias.md)).
...
- **Clasificación en bandejas (`box`).** La prioridad de asignación es **`TRASH` > `SPAM` > `SENT` > (en bandeja de entrada) `ALL_MAIL` > `ARCHIVE`**, idéntica para ambos proveedores, pero parte de orígenes distintos: en Gmail de **etiquetas** (un correo puede llevar varias a la vez) y en Outlook de **carpetas**. El matiz nuevo: un correo recibido cuenta como `ALL_MAIL` **solo si sigue en la bandeja de entrada** del proveedor (Gmail: tiene la etiqueta `INBOX`; Outlook: está en la carpeta `inbox`); si no la tiene (y no es spam/papelera/enviado), cae en `ARCHIVE`. Antes ese caso caía en `ALL_MAIL`, por eso los correos archivados en el proveedor se colaban en la bandeja principal — ahora se reclasifican como archivados en la siguiente sincronización (sin backfill: el cambio es gradual por cuenta). El resultado para el usuario es el mismo: cada correo cae en una sola bandeja navegable.
...
- Ordenar y filtrar el listado (orden por fecha/remitente/asunto, chips no leídos/con adjuntos/destacados): [../features/ordenar-y-filtrar-listado.md](../features/ordenar-y-filtrar-listado.md) · [./ordenar-y-filtrar-listado.md](./ordenar-y-filtrar-listado.md)
