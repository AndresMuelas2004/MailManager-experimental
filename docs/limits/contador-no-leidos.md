...
|---|---|---|---|
| Bandejas con badge | **2**: bandeja de entrada (`ALL_MAIL`) y spam (`SPAM`) | Parámetro `box` del endpoint de conteo (por defecto `ALL_MAIL`) | Cualquier otro valor de `box` se rechaza con **422**. Son las únicas bandejas con noción de "no leído" en la UI; Enviados, Favoritos, Borradores, Papelera y bandejas ficticias **no** tienen badge. |
...
| Superficies que muestran el badge | **4 dentro de la app** + **1** el título de la pestaña | Menú lateral (Bandeja unificada, Spam), pestañas de cuenta (Bandeja, Spam), tarjeta de cuenta conectada (solo bandeja de entrada), título del navegador | La **tarjeta de cuenta** muestra solo el de **bandeja de entrada** (no el de spam). |
| Ámbito del conteo del menú lateral | Suma de **todas las cuentas** del buzón actual | Backend: `total` del buzón | El menú lateral usa el total del buzón; las pestañas y las tarjetas usan el desglose **por cuenta**. |
| Desglose por cuenta | **Todas** las cuentas del buzón, incluidas las que tienen **0** | Backend: lista `accounts[]` con un `unread` por cuenta | Una sola respuesta por `(buzón, box)` alimenta a la vez el total del menú, las pestañas por cuenta y las tarjetas. |
...
- **No hay badge en Enviados, Favoritos, Bandejas ficticias, Borradores ni Papelera.** *Por qué:* ninguna tiene una noción natural y única de "no leído" — los enviados y borradores son del propio usuario, y Favoritos/Papelera/ficticias son vistas transversales que cruzan bandejas. Es una decisión de producto.
