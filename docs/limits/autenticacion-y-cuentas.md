...
| Espera máxima del callback OAuth al conectar Outlook | **300 segundos** | Backend (flujo interactivo de conexión) | Si el consentimiento en el navegador no se completa en ese plazo, la conexión aborta con un error de OAuth (timeout esperando el callback). **Asimetría**: el flujo de Gmail abre un servidor local en un puerto efímero gestionado por la librería de Google y **no** impone este timeout de 300 s; el límite aplica solo a Outlook. |
| URL de redirección OAuth de Outlook | **Debe ser `http://localhost` o `http://127.0.0.1`** | Backend (conexión de Outlook) | El `redirect_uri` configurado para Outlook se valida al conectar; cualquier otro esquema/host hace fallar la conexión con `account_misconfigured` (400). Es una restricción del flujo OAuth local del MVP. |
