...
- **La marca solo avanza en una sincronización resuelta (no rechazada).** Un **fallo total** —ninguna cuenta pudo sincronizar, la operación se rechaza— deja la marca intacta. En cambio, un **éxito parcial** de la bandeja unificada (al menos una cuenta va bien aunque otra esté caída) **sí** avanza la marca, porque la operación se resuelve con normalidad y trae correo real. La **bandeja ficticia**, con su abanico tolerante a fallos parciales, también avanza la marca en cuanto el intento termina aunque alguna cuenta falle por debajo. (Ver el comportamiento en el gemelo § 5.1.)
...
| **Control de refresco en Favoritos** | La página de Favoritos **no** recibe este control | Los favoritos se capturan solos en la sincronización general y el antiguo botón "Sincronizar favoritos" se retiró de la UI, así que Favoritos ya no lleva ningún botón de sincronización ni el control "Refrescar"/"última actualización" (ver [../features/favoritos.md](../features/favoritos.md)). |
| **Avisos de fallo en la bandeja ficticia** | La bandeja ficticia no muestra ni el rojo "No se pudo actualizar" ni el aviso ámbar que nombra la cuenta caída (ese aviso ámbar es **exclusivo de la bandeja unificada**); avanza la marca aunque una cuenta falle | Su sincronización tolera fallos parciales por diseño y **en silencio**; frenarse o avisar por la cuenta más débil contradiría el valor de agregar lo que sí se pudo traer. |
