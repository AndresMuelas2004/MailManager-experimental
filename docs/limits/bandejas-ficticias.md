...
- **Exclusión por defecto de papelera, spam y archivados**: si no se indica ni `box` ni `box_not_in`, la bandeja excluye `TRASH`, `SPAM` y `ARCHIVE` automáticamente. Para incluirlos, hay que enviar `box_not_in: []` ("no excluir nada").
- **`ARCHIVE` se excluye SOLO por defecto (asimetría con `DELETED`)**: a diferencia de `DELETED` —que se añade a **todas** las ramas de exclusión—, `ARCHIVE` se añade **únicamente** a la exclusión por defecto. No se fuerza ni sobre un `box_not_in: []` ("no excluir nada") ni sobre una lista de exclusión explícita: si el usuario pide ver todo, o fija su propia lista, se respeta literalmente. Además, `ARCHIVE` es el **único** box excluido por defecto que un `in:archive` explícito en la lupa **rescata** (el listado lo lleva a `box="ARCHIVE"` en vez de cortocircuitar a página vacía); `in:trash` / `in:spam` sobre una ficticia por defecto siguen dando vacío. `ARCHIVE` **no** es un valor seleccionable de `box`/`box_not_in` (no es miembro de `FilterBox`), así que no se puede guardar como filtro de la ficticia — solo es alcanzable vía `in:archive`.
- **Exclusión permanente de los borrados (`DELETED`)**: el estado interno `DELETED` (correo eliminado de forma definitiva tras vaciarlo) se excluye **siempre**, incluso con `box_not_in: []`. No es un valor de carpeta seleccionable (no está entre los admitidos de `box`/`box_not_in`), así que el usuario no puede pedirlo; se oculta en todas las ramas de filtro. A diferencia de la de papelera/spam/archivados, esta exclusión **no** se puede desactivar.
...
| **Selección múltiple y acciones en bloque** | No hay un "vaciar bandeja ficticia" ni casillas de selección | A diferencia de la cuenta/unificada (que sí conservan la selección al agrupar), la ficticia reúne cuentas de varios buzones sin una caja única a la que dirigir el lote, así que su fila es de solo lectura; las acciones (papelera, leído, favorito) operan **por mensaje** desde el visor de la conversación, sobre los correos reales subyacentes (ver [conversaciones.md](conversaciones.md)). |
...
| Bandeja con `box` = `TRASH`/`SPAM`/`SENT` | Muestra solo esa carpeta, ignorando la exclusión por defecto | `box` explícito tiene prioridad sobre el "excluye papelera/spam/archivados por defecto". |
| Correos **archivados** (`ARCHIVE`) en una ficticia por defecto | **No** aparecen por defecto, pero un `in:archive` en la lupa los **rescata** | `ARCHIVE` se excluye **solo** por defecto (no sobre `box_not_in: []` ni sobre una exclusión explícita — § 2). Es el único box excluido por defecto recuperable con `in:` (`in:trash` / `in:spam` siguen dando vacío). No es un filtro guardable (no es miembro de `FilterBox`). |
