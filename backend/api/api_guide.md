...
### Rate-limit dependencies — `rate_limit_by_ip` / `rate_limit_by_user`, global at router level

Two `Depends` factories in `routers_helpers.py` throttle a route via the pure engine in `api/rate_limit.py`: `rate_limit_by_ip(bucket)` keys by the client IP (login + the `global` safety net), `rate_limit_by_user(bucket)` keys by the authenticated user (`Depends(require_session)`, so it shares the integration test's auth override and FastAPI runs `require_session` once per request even when the route also depends on it). Both are **no-ops unless `RATE_LIMIT_ENABLED` is truthy** (read per request, so a test can `monkeypatch.setenv` without rebuilding the app). The `global` per-IP net is applied **once per request at router level** in `app.py` — one shared `Depends` on every router **except** `health_router` and the OAuth `callback` router (a single router holding both the Google and Outlook callback routes; a 429 on the provider-driven callback would break a legitimate account connection, not abuse — do NOT add the global dep there). A throttled bucket raises `TooManyRequests` with `detail = {"scope", "retry_after"}`; the generic `ApiError` handler turns any `detail["retry_after"]` into a `Retry-After` **header** (and keeps it in the JSON body — the header is not CORS-exposed, so the cross-origin SPA reads the countdown from the body, same trap as the download filename). The opt-in rationale, the in-memory/single-worker limit, the `X-Forwarded-For`-first-hop identity, and the shared-bucket map (`auth_login` = `POST /auth/google` + `POST /auth/microsoft` + `POST /auth/dev-login`, `email_send` = direct + draft send, `provider_sync` = sync-metadata + favorites/sync + drafts/sync) live in `repository_guide.md`; the figures are the `RATE_LIMITS` dict.
...
- **Status quirks worth remembering:** `DatabaseQueryError` is 503 (transient-from-the-caller perspective, retryable), not 500. `EmailListError` / `DraftListError` / `UnreadCountError` are 500 because a listing / counting failure is the only place that specific operation can fail and there is no retry story (a `DatabaseError` from the same store call still translates to 503 — `UnreadCountError` fires only on the `except Exception` branch). 409s (`EmailNotInTrash`, `AccountNotConnected`, `AccountConnectAuthError` is 401) encode state conflicts, not plain missing resources — do not downgrade them to 404 when reusing.
...
- **Rate limiting:** `TooManyRequests` is **429** (code `rate_limit_exceeded`). Unlike every other class here it is raised **from a router dependency** (`rate_limit_by_ip` / `rate_limit_by_user` in `routers_helpers.py`), not from a service — the limiter must run before the route function, so the "services are the only raisers" convention does not reach it. Its `detail = {"scope", "retry_after"}` is what makes the generic handler emit the `Retry-After` header (see the rate-limit trap above).
...
- `POST /auth/dev-login` is a test backdoor, not a provider flow. It bypasses OIDC entirely (no `verify_*_token`, no `AuthSettings.client_id`, no `translate_auth_error`) and must NOT be used as a template when adding a real identity provider — copy from `google_login` instead. It is also the **only** login path that resolves identity by **email** (`user_store.get_by_email`), never by the composite `(auth_provider, provider_sub)`; under "no account-linking" two users can share an email, so `GET_USER_BY_EMAIL` (`LIMIT 1`, no `ORDER BY`) may return either non-deterministically — inert in practice because it only runs against the configured `DEV_LOGIN_EMAIL` user.

**`google_login` vs `microsoft_login` — client-id config asymmetry (do not "unify").** `GOOGLE_CLIENT_ID` is required in `AuthSettings` (`get_auth_settings` raises `AuthSettingsError` when absent), so `google_login` can assume it. `MICROSOFT_CLIENT_ID` is **optional in settings** (defaults to `""`) and the "is Microsoft login configured?" check lives **at the point of use**: `microsoft_login` raises `EnvVarError` (500 `env_var_error`) when it is empty, **before** calling `verify_microsoft_token`. This is deliberate — a Google-only deploy (and every test that only sets `GOOGLE_CLIENT_ID`) must still load settings and boot; only an actual Microsoft login attempt fails. Two further service-side identity rules `microsoft_login` applies that `google_login` does not (mirrored in `auth_guide.md`): the user email falls back to `preferred_username` when `email` is absent (Entra does not guarantee `email` on `common`), and `email_verified` is **not** checked (Entra does not emit it). The identity persisted is always the `sub` claim → `provider_sub`, never the email. **Upsert-key trap for a 3rd provider:** the dict passed to `user_store.upsert` MUST carry **both** `auth_provider` (the literal `"google"` / `"microsoft"`) and `provider_sub`; the UPSERT conflict target is the composite `(auth_provider, provider_sub)` (migration 0037), never `provider_sub` alone. A provider that omits `auth_provider`, or assumes `provider_sub` is the unique key, silently breaks login idempotency (re-login mints a new user / collides across providers).
