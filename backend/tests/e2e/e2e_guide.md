...
## Cobertura del backfill-status — `test_46q`

`GET .../backfill-status` es solo-BD (sin llamada al provider), montado en `mailboxes_router` bajo ÚNICAMENTE el rate-limit global — sin el bucket dedicado `provider_sync` que sí llevan `sync-metadata` y `drafts/sync`. El backfill real se encola SOLO desde el flujo interactivo `POST .../connect` (ya excluido de la suite) y corre durante horas contra una cuenta real, así que el bucle del worker en sí queda cubierto solo por las suites unit + integration. `test_46q` verifica ÚNICAMENTE el contrato de LECTURA contra las cuentas sembradas ya sincronizadas (sin job de backfill activo): `active` es `False`, y cada entrada de `accounts[]` (si la hay) lleva `status` en `{completed, failed}` y `done=True`. Las cuentas sin ningún job simplemente no aparecen en `accounts[]` — comportamiento del schema, no un hueco del test.

## Cobertura de favoritos — `test_favorites_flow_gmail.py` / `test_favorites_flow_outlook.py` (`test_58`–`test_63`)

Las dos suites ejercitan `PATCH .../favorite`, `POST /favorites/sync` y `GET /emails?favorite=true` contra el provider real, un fichero por implementación en `backend/core/email/`. El contrato portante es **la restauración de estado, no las aserciones**: el toggle de favorito muta estado real del provider (Gmail `STARRED` / Outlook `flag`), así que cada test de toggle captura el `is_favorite` original de la fila elegida y lo restaura en un bloque `finally` — la cuenta de test sembrada debe verse idéntica antes y después de la ejecución (la regla de "los datos de test preexistentes son sagrados" aplica también a la marca de favorito, no solo a las filas/cuentas). El toggle es Provider-First e idempotente, así que la restauración siempre es segura. Las aserciones de RECUENTO del sync son intencionadamente laxas (`favorites_synced <= total_synced`, ambos `>= 0`) porque el recuento de favoritos de la cuenta viva no es fijo; no las aprietes a un número exacto contra un inbox mutable. Pero ambos tests de sync llevan además una aserción **portante de supervivencia**: marcan un favorito vía el toggle justo antes del sync y afirman que sigue `TRUE` después (restaurando en `finally`). Existe porque `sync_favorites_for_account` es un reemplazo total (`is_favorite = pmid = ANY(ids_del_provider)`): los ids que devuelve la ruta `$filter=flag/flagStatus` de Outlook no casan con los ids delta almacenados (la misma deriva de ids por-endpoint verificada en vivo para `$filter=conversationId` — ver `external-apis-used/Outlook/08`), así que sin reconciliación el sync barrería TODOS los favoritos de la cuenta a FALSE y el sobre laxo se quedaría en verde. `favoritos.py::_reconcile_favorite_ids` cierra esa deriva (ver `repository_guide.md` / `core_guide.md`), y el lado Outlook de este test es ahora su **test de regresión en vivo**: un rojo en la aserción de supervivencia significa que la reconciliación dejó de funcionar contra la API real (p. ej. Graph cambió la forma del `$select` enriquecido, o los campos de identidad dejaron de coincidir con lo que persiste el delta-sync) — la respuesta correcta es investigar la reconciliación, nunca relajar la aserción. El camino de retry de Outlook (`set_favorite` / `list_favorite_ids` ahora reintentan el throttling transitorio honrando `Retry-After`) **no** es directamente afirmable aquí — solo se activa ante un hipo real del provider; su cobertura determinista vive en la suite unit. Los tests de listado fijan `group_by_thread=false` deliberadamente: Favoritos está **siempre desagrupado** por decisión de producto (el backend no aplica ningún override de agrupación acoplado a `favorite`, así que `favorite=True`+`group_by_thread=True` es SQL válido de "agrupa los favoritos" que simplemente no es una superficie de producto). El `false` hardcodeado es la especificación ejecutable de esa decisión — no lo "arregles" al valor por defecto del router ni lo voltees para ejercitar la agrupación, lo que cambiaría silenciosamente lo que el test fija.
