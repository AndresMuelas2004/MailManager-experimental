...
Dos entradas de la lista `isolated_db` cargan una consecuencia no obvia más allá del aislamiento rutinario. (1) `sync_email_metadata` reconcilia pertenencias de carpeta y evalúa reglas en CADA sync (`folder_store.get_ref_map` / `rule_store.list_active_by_owner`; la 2ª como background task que el `TestClient` corre síncrono — carpetas-y-reglas), así que **todos** los tests de sync — no solo los endpoints dedicados de carpetas/reglas — tocan los repos `folder`/`rule`/`rule_apply`; sin sus parches un test de sync pre-existente filtra filas por el pool real. (2) la clave de `image_proxy_cache` es GLOBAL (no por-cuenta/usuario), así que una fila filtrada de un test convierte un cache-miss posterior en un falso cache-hit.

### Trampa 2 — un nuevo módulo (o submódulo) de servicio debe parchearse en `_apply_test_monkeypatches`

`_apply_test_monkeypatches` parchea `build_manager_for_accounts`, `load_wrapped_app_credentials`, `load_wrapped_account_tokens` y `account_store.upsert_tokens` **por-módulo, en cada módulo que enlaza el nombre — nunca sobre la fachada de un paquete**. Tras el split de `emails_service` / `drafts_service` / `services_helpers` a paquetes, parchear la fachada `emails_service` NO intercepta (misma trampa de patch-target que `unit_guide.md`): el parcheo recorre submódulo a submódulo las tuplas `_EMAILS_/_DRAFTS_BUILD_MANAGER_MODULES` y `_EMAILS_/_DRAFTS_LOAD_WRAPPED_MODULES`, más `accounts_service`, `attachments_service` y `services_helpers.carpetas` (que enlaza los nombres para la ruta de asignar/desasignar carpeta por-email — carpetas-y-reglas). **Asimetría load-bearing:** el conjunto de submódulos que enlaza `build_manager_for_accounts` NO coincide con el que enlaza `load_wrapped_*` — en `emails_service` los nueve submódulos de `_EMAILS_BUILD_MANAGER_MODULES` construyen el manager, pero SOLO `_comunes` está en `_EMAILS_LOAD_WRAPPED_MODULES`, así que un submódulo nuevo debe añadirse a la tupla correcta según qué nombres enlaza de verdad. Omitir cualquiera hace que sus tests golpeen las APIs reales del proveedor.

### Trampa — el worker de backfill se apaga en la suite (`BACKFILL_WORKER_ENABLED=false`)

El worker de backfill arranca un hilo daemon en el lifespan de la app (que el `TestClient` de scope de sesión entra). `conftest.py` fija `BACKFILL_WORKER_ENABLED=false` vía **asignación dura** (`os.environ[...] = "false"`, NO `setdefault`) **en el import del módulo** (antes de que el lifespan corra) para que ningún hilo de fondo sondee la BD real durante la suite — la asignación dura es deliberada: un `setdefault` dejaría que una var ambiental `BACKFILL_WORKER_ENABLED=true` encendiera el daemon y volviera la suite no determinista (§4.8 tratado como invariante absoluto). La consecuencia portante: la guarda del sync que excluye las cuentas con backfill activo es **independiente del flag**, así que sus tests (job `running` sembrado → la cuenta no se sincroniza; job `completed` sembrado → un `is_full_sync` NO dispara la reconciliación de fantasmas para ella) la ejercitan por completo SIN lanzar el hilo. Los tests del **encolado-en-connect** son la excepción: hacen `monkeypatch.setenv('BACKFILL_WORKER_ENABLED', 'true')` por caso para probar que un connect con `sync_cursor` NULL crea el job `pending` (y que con no-NULL, o con el flag off, no lo crea). Un test que asuma el flag ON por defecto, o que dependa de que el hilo procese jobs, es no determinista. (El nuevo `account_backfill_repo_module` ya está en la lista de monkeypatch de `isolated_db` — Trampa 1 arriba; sin él las lecturas/escrituras del job se escapan de la transacción por test.)
...
- **`draft_sync_jobs` (migración 0042) es un clon de `account_backfill_jobs`** — misma forma de tabla + índice parcial `idx_draft_sync_jobs_active` + FK UUID `ON DELETE CASCADE` + reaper `reset_retriable_failed_to_pending` (con sus mismos negativos: permanente `attempts>=max` y fresco dentro del cool-off), y el connect encola AMBAS colas cuando el worker está enabled. La ÚNICA asimetría load-bearing: su `ENQUEUE` resetea a `pending` CUALQUIER status **incondicionalmente** (sin el `WHERE status='failed'` del backfill), porque una reconexión debe refrescar siempre los borradores — `test_draft_sync_enqueue_resets_any_status_unconditionally` lo fija reencolando sobre `completed` Y `running` (ambos → `pending`), justo los estados que el backfill deja intactos.
...
### Favoritos — siete trampas no cubiertas en otro lugar (`test_favorites.py`)
...
- **`is_favorite` es provider-autoritativo: se propaga por el SYNC, no solo por el toggle.** Un mensaje sincronizado/backfilled con la estrella puesta aterriza con `is_favorite=True` (`test_sync_persists_is_favorite_and_does_not_wipe` prueba que la tupla del upsert está alineada con su columna), y el incremental lo actualiza vía `LabelUpdate`. ⚠️ Asimetría COALESCE (la trampa load-bearing): `UPDATE_LABELS_BATCH` aplica `is_favorite = COALESCE(v.is_favorite::BOOLEAN, em.is_favorite)`, así que un delta parcial de Outlook SIN `flag` (envía `is_favorite=None`) NO borra un favorito existente, mientras que un bool concreto (Gmail siempre, Outlook con `flag`) lo pisa. Un test que envíe `is_favorite=None` esperando que borre la estrella verifica lo contrario del contrato.
- **`test_sync_favorites_outlook_reconciles_drifted_id` inyecta el drift vía `list_favorite_candidates_return`, NO `list_favorite_ids_return`.** El `FakeEmailClient` sigue soportando `list_favorite_ids_return` (Gmail, otros tests de este fichero) porque el default de la ABC lo envuelve sin campos de identidad; para simular el per-endpoint id drift de Outlook hay que inyectar `FavoriteCandidate`s enriquecidos directamente (helper `build_favorite_candidate`), o el fake se comporta como Gmail y la reconciliación se salta por completo (short-circuit de "ningún candidato trae identidad").
