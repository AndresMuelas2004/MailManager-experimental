...
### Trap — `test_auth_endpoints.py`: Microsoft tests need BOTH client-id env vars; the seed is keyed by `(auth_provider, provider_sub)`

- **`_set_ms_env` sets BOTH `GOOGLE_CLIENT_ID` and `MICROSOFT_CLIENT_ID`.** `_load_auth_settings` demands `GOOGLE_CLIENT_ID` regardless of which provider is under test, so a Microsoft test that sets only `MICROSOFT_CLIENT_ID` 500s with `env_var_error` pointing at the *wrong* missing var. `test_microsoft_login_not_configured` asserts the inverse (GOOGLE set, MICROSOFT absent → 500) AND that `verify_microsoft_token` is never reached (the point-of-use guard fires first).
- **`_seed_test_user` upserts on `(auth_provider, provider_sub)` (migration 0037); `TEST_USER_GOOGLE_SUB` is the load-bearing `provider_sub`.** A raw insert of a second user with the SAME email but a different provider/sub succeeds and creates a DISTINCT row — the intended "no account-linking" model, but a surprise for any ownership test that assumes one user per email. The 0037 identity contract is pinned end-to-end by `test_login_idempotent_same_provider_sub_returns_same_user` (same provider+sub → same `user_id`, one row) and `test_same_email_two_providers_creates_distinct_users` (Google + Microsoft, same email → two `user_id`s).

### Trap — `test_contacts_suggestions.py`: not `seeded_test_client`, and error injection is two-stage

- **The migration-0010 seed cannot exercise this endpoint** — it predates migration 0031, so every seeded row has empty `to_email`/`to_name` and a `from_email` equal to the account's own address (dropped by the own-address exclusion). The tests stage their own rows and use `test_client_base`: the service makes NO provider call, so no fake manager is needed.
- **The two store calls fail on different preconditions.** `account_store.list_account_ids_by_user` failure fires even with zero accounts; `list_recipient_suggestions` failure requires at least one owned account to get past the account-lookup stage. A suggestions-store error test that omits account setup short-circuits at the lookup stage and asserts the wrong error path. Whitespace-only `q` (passes `min_length=2` but tokenises to nothing) returns 200 `[]`, not 422.
...
### Trap — `ARCHIVE` in a vmbox: default-excluded, `in:archive`-rescued, never a saved filter (three tests in lockstep)

`ARCHIVE` joins TRASH/SPAM in a fake mailbox's DEFAULT exclusion, but it is the ONE default-excluded box that an explicit `in:archive` is allowed to RESCUE (TRASH/SPAM/DELETED collapse to empty instead). And it is deliberately NOT a `FilterBox` member, so `box:"ARCHIVE"` / `box_not_in:["ARCHIVE"]` saved in a vmbox filter are 422 at the schema boundary — archived mail is reachable inside a vmbox ONLY via `in:archive`. Three tests guard the terna and must move together: `test_emails_for_vmbox_default_excludes_archive` (a seeded ARCHIVE row stays hidden by default, `total == 0` via a unique `subject_contains`), `test_emails_for_vmbox_in_archive_rescues_archived_rows` (`in:archive` surfaces it), and `test_create_rejects_box_archive_filter` + `test_create_rejects_box_not_in_archive_filter` (both fields → 422). The rescue is scoped to the default-exclusion branch only: `test_emails_for_vmbox_in_archive_on_pinned_box_returns_empty` pins that a vmbox pinned to a different `box` (e.g. SENT) takes the `box is not None` branch where `in:archive` is incompatible and collapses to an empty page — a refactor that moved the `ov == "ARCHIVE"` rescue out of the `box_not_in` branch would leak archived rows into a box-pinned vmbox without any other test catching it.
...
Two reply-resolution edge cases the default fixture does not exercise: a base email in the **SENT** box whose `from_email` is the user's own address resolves the reply `To` from the original `to_recipients` (self-reply), not from `from_email`; and a non-empty `reply_to` wins over `from_email` for the reply `To` (R-10). A test for either must populate the corresponding `ReplyContext` field — neither is reachable from the standard seed.
...
- **Both inline and downloadable attachments surface in the response** (`LIST_EMAIL_ATTACHMENTS_BY_MESSAGE` does not filter `is_inline` — hiding inline parts is a frontend concern). A cache-hit attachment test asserting only the downloadable row is wrong.
- **The cache-HIT TTL test must seed `fetched_at` in the past** (e.g. `now() - 60 days`) so the assertion that the touch bumps `last_accessed_at` but NOT `fetched_at` is measurable — with both at `now()` the test passes even if `fetched_at` were bumped.
...
### Rate-limit tests (`test_rate_limit.py`) — TestClient IP collapse + shared buckets

Under `TestClient` every request reports `request.client.host == "testclient"` with no `X-Forwarded-For`, so `client_ip()` returns ONE key for all requests in a test (same root cause as the `test_dev_login.py` host trap above). For the limiter this means setup POSTs and create-draft calls also charge the per-IP `global` bucket, so a test targeting a SPECIFIC bucket keeps `global` generous (e.g. `(50,60)`) to avoid a spurious 429, while the test for `global` itself uses a small value. Counters are module-level state cleared by `_enable` and the `_isolate_counters` autouse fixture; the default-OFF test must NOT call `_enable` — it proves the no-op by the ABSENCE of the env var.

The load-bearing assertions are the shared-bucket ones: `provider_sync` (sync-metadata + favorites/sync + drafts/sync) and `email_send` (direct send + draft send) are each verified across ALL their members by exhausting the bucket on one member and asserting another also 429s under the same `scope`. A per-endpoint test would pass while a split silently widened the real ceiling — only the cross-endpoint assertion catches it. The OAuth-callback and `/health` exemptions are each proven behaviourally by hammering the endpoint past a small `global` and asserting it never 429s.
