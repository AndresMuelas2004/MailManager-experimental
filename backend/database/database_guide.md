...
- **The `EXCLUDED`/INSERT lockstep is load-bearing.** `signature_html` must appear in **all four** clauses of `UPSERT_ACCOUNT` — INSERT column list, `VALUES`, `DO UPDATE SET`, and `RETURNING`. Listing it only in `DO UPDATE SET` (the tempting minimal edit) makes `EXCLUDED.signature_html` resolve to the column DEFAULT (`NULL`) because nothing was inserted for it, so **every account update would silently wipe the signature to NULL**.
- **It uses plain `EXCLUDED.signature_html`, deliberately NOT `COALESCE` like `email_address`.** The asymmetry is safe only because `UPSERT_ACCOUNT` is reached **exclusively** through `PgAccountStore.upsert` (i.e. `create_account` + `update_account`), never through the silent token-refresh path (`upsert_tokens` runs the separate `UPSERT_TOKENS_*` queries). `email_address` needs COALESCE precisely because the refresh path hits its UPSERT without carrying the value; the signature never travels that path, so the latest write always carries the intended value. `create_account` omits the key entirely and relies on `upsert`'s `setdefault("signature_html", None)` — adding COALESCE here would be dead weight, while a new caller routing token refreshes through `UPSERT_ACCOUNT` would resurrect the blank-on-refresh bug.
...
## `users` identity is `(auth_provider, provider_sub)`, not email — and `GET_USER_BY_EMAIL` is `LIMIT 1` by consequence (migration 0037)

The user identity is the composite **`(auth_provider, provider_sub)`** (UNIQUE + `CHECK (auth_provider IN ('google','microsoft'))`), NOT the email. There is **no account-linking**: the same person logging in once with Google and once with Microsoft under the same mailbox address produces **two distinct user rows** (different `provider_sub`, possibly equal `email`). The UPSERT conflict target is therefore `(auth_provider, provider_sub)` — never `email`. `GET_USER_BY_EMAIL` keeps its `LIMIT 1` precisely because `email` is no longer unique: it may now return either of two same-email users non-deterministically. Its only caller is the dev-login backdoor (which mints a session for `DEV_LOGIN_EMAIL`); regular login never looks a user up by email, so the ambiguity is inert outside dev. A future account-linking feature must add a real disambiguator here, not assume one-user-per-email.

## Migration 0037 generalize-`users` — the `DO`-block is gated by `google_sub` for idempotency

Migration 0037 (`down_revision = "0036_email_content_ttl_and_truncate"`) rewrites the pre-0037 `users` shape (single `google_sub UNIQUE NOT NULL`) into `(auth_provider, provider_sub)`. In the **runner** (`migrations/runner.py`) the upgrade is a `DO $$ … $$` block wrapped in `IF EXISTS (… column_name = 'google_sub')` — load-bearing for re-run safety: `ALTER TABLE … ADD CONSTRAINT … UNIQUE` does **not** accept `IF NOT EXISTS` in PostgreSQL, so a second pass over an already-migrated DB would crash with "constraint already exists" without the guard. On a fresh bootstrap the inline `CREATE TABLE users` already carries the final shape and `google_sub` never existed, so the block is a clean no-op there too. The lockstep is four-way and all four must move together: the Alembic `versions/0037_*.py`, the runner's inline `CREATE TABLE` + gated `DO`-block + stamp, the `schema.sql` snapshot, and the migration-0010 seed (its `INSERT` and `ON CONFLICT` now list `(auth_provider, provider_sub)`). See `repository_guide.md` § "Extension / Alembic migration" for the general runner rule.

## Trash — `previous_box` + `DELETED` box value (migration 0008)

- `previous_box VARCHAR(20)` nullable, CHECK allows `ALL_MAIL`, `SENT`, `SPAM`, `ARCHIVE` (the last added in migration 0038 — see below). `move_to_trash_batch` copies the current `box` into `previous_box` before setting `box = 'TRASH'`. `restore_from_trash_batch` uses `COALESCE(previous_box, 'ALL_MAIL')`. Rows where `previous_box IS NULL` go through `restore_from_trash_discovered_batch`, which receives the discovered box from the caller.
...
## `ARCHIVE` box value (migration 0038)

`ARCHIVE` is a mutually-exclusive `box` location (like `SENT`/`SPAM`/`TRASH`), added to the `box` CHECK. It also had to be added to the **`previous_box`** CHECK in the same migration, which is the non-obvious half: archiving then trashing copies `box='ARCHIVE'` into `previous_box` (`MOVE_TO_TRASH_BATCH`), which the pre-0038 `previous_box` constraint (`ALL_MAIL|SENT|SPAM`) would reject — so trashing an archived message would 500 without extending both checks. Extending `previous_box` also lets `RESTORE_FROM_TRASH_BATCH` (`COALESCE(previous_box,'ALL_MAIL')`) restore such a message back to `ARCHIVE`. No data backfill / cache truncate: already-archived provider mail reclassifies into `ARCHIVE` gradually on the next per-account sync (Gmail recomputes from labels, Outlook from `parentFolderId`).
...
### `email_content` sliding-TTL purge — the `IS NOT NULL` guard the blob purge needs is deliberately ABSENT here

`PURGE_EXPIRED_FOR_ACCOUNTS` (migration 0036) is the body-cache sibling of `PURGE_EXPIRED_BLOBS`, with the **same** 30-day interval, but its predicate is just `last_accessed_at < now() - INTERVAL '30 days'` — **no `IS NOT NULL` half**, and that omission is correct, not an oversight. `email_content.last_accessed_at` is `NOT NULL DEFAULT now()` (every row IS a cache entry, created and "accessed" at persist time), whereas the attachment column is nullable (a part can be listed but never downloaded). Copying the blob purge's `IS NOT NULL` guard here would be dead weight; dropping it there would purge un-downloaded binaries early. Do not normalise the two predicates. The query is scoped per account (`account_id = ANY(...)`) because the purge is hooked to sync, account-by-account — backed by the composite `idx_email_content_last_accessed (account_id, last_accessed_at)`. The companion `TOUCH_LAST_ACCESSED` (cache-hit refresh) touches `last_accessed_at` **only** — never `fetched_at` (a read is not a re-fetch; see `repository_guide.md` for the E2E assertion that depends on this).

### `LIST_UNREAD_RECENT_UNCACHED` — content-prefetch target select, no dedicated index by design

Backs the sync-time content prefetch (see `repository_guide.md`): the unread / recent-≤48h / `box='ALL_MAIL'` messages whose body is not yet cached, via a `LEFT JOIN email_content … WHERE ec.provider_message_id IS NULL` anti-join, `ORDER BY received_at DESC LIMIT %(limit)s`. **No new index** (head stays where the prefetch query needs nothing extra): the existing `idx_email_metadata_received_at` + `idx_email_metadata_account_id` cover the predicate at MVP volume and the select runs at most once per account per sync. If a user ever syncs hundreds of thousands of messages the candidate is a partial `(account_id, received_at DESC) WHERE is_read = FALSE` (twin of `idx_email_metadata_favorites`) — not built today. A malformed `account_id` collapses to `[]` (treated as "no results", like `exists`), so the prefetch never 500s on bad input.
...
### `LIST_FILTERED` external ORDER BY is a `{order_by}` slot — total-ordering tie-break + alias trick

The four LIST templates' **external** `ORDER BY` is a `str.format` slot (`{order_by}`) filled by `_build_order_by(sort, sort_dir)` from the closed `_SORT_EXPRESSIONS` whitelist — never the raw `sort` value (the anti-injection discipline of `_EXTRA_FILTER_BUILDERS` / `_OPERATOR_CLAUSE_BUILDERS`). The `sort`/`sort_dir` params are wired by the regular `GET /emails` ONLY; the vmbox listing calls `list_filtered` without them → `None`/`None` → date-desc, semantically identical to the pre-slot fixed order.

Three things the slot does not reveal:

- **The PK tie-break (`account_id, provider_message_id`) is always appended**, in every `_build_order_by` branch — total by construction (the trailing pair is the table's PK). Drop it and OFFSET paging goes non-deterministic when two rows share `received_at` (mass-sent notifications batched to the same second): the same row can appear on adjacent pages or be skipped. Single-page tests never catch this; the regression only manifests when the user scrolls. A non-date sort additionally inserts a secondary `received_at DESC` before the tie-break.
- **The slot body is UNPREFIXED on purpose.** `received_at` / `from_name` / `from_email` / `subject` exist only in `em` (the JOINed `accounts` has none of them) and `account_id` is the `USING`-merged column, so unprefixed references resolve to `em.*` in `LIST_FILTERED`; the three subquery templates expose the same columns through their single outer alias `d`, so the SAME unprefixed body resolves to `d.*` there. One `_build_order_by` therefore serves all four templates with no alias parameter — which is why the SQL-surface test now asserts the unprefixed `ORDER BY received_at DESC, account_id, provider_message_id` (the pre-feature `d.`-prefixed form is gone).
- **Only the SELECT carries `{order_by}`; the COUNT templates do NOT.** `count_filtered` takes no `sort`/`sort_dir` and the COUNT constants have no `{order_by}` slot — a COUNT does not order, and an unfilled slot would `KeyError`. The chips (`unread`/`has_attachment`/`favorite_only`) are the inverse: they arrive as ordinary `operator_clauses`, so they reach BOTH `list_filtered` and `count_filtered` unchanged (the `total` must count exactly the filtered page). The internal `DISTINCT ON` / windowed winner-selection `ORDER BY`s are untouched by the slot — they pick which row represents each group, independent of presentation order.
...
### `COUNT_UNREAD_BY_ACCOUNT` — unread-badge counter, per-account in one round trip, no index by design

Backs `email_metadata_store.count_unread_by_account` (the unread-badge endpoint — see `repository_guide.md`). It returns the per-account `{account_id: unread}` breakdown in a **single** `GROUP BY account_id` round trip rather than N× `count_filtered` (one per account), which would be N round trips. ⚠️ A `GROUP BY` emits **no row for accounts with zero unread**, so the service MUST fill 0 for the rest of the mailbox — the repository contract is "only the non-zero accounts come back". Counts INDIVIDUAL messages (no thread grouping, by product decision). `box` is a single trusted value bound as a parameter (the API `Literal` constrains it to `ALL_MAIL`|`SPAM`); the `account_ids` `::uuid[]` cast and the empty-list short-circuit / `InvalidTextRepresentation → {}` are the same traps as `LIST_FILTERED`. **No dedicated index** (head unchanged): the `account_id` equality + `is_read`/`box` filter rides `idx_email_metadata_account_id` at MVP volume, the same trade-off as the filtered listings and `LIST_RECIPIENT_SUGGESTIONS`.
...
**`migrations/runner.py` must be updated in the same change**: append the equivalent DDL to `_DDL_STATEMENTS` and advance the stamp at the bottom to the new migration name. Forgetting this silently breaks any environment that relies on the fallback runner (local setup without Alembic, some CI configurations). Data-only migrations also belong here — e.g. migration 0014 adds `TRUNCATE TABLE email_content;` immediately before the stamp line, and every subsequent `email_content`-invalidating migration follows the same shape (see `repository_guide.md` § "Email HTML rendering cache"). Migration **0034** (rich-text composer) is data-only, but it is a textual **conversion** rather than a `TRUNCATE`: a single idempotent `UPDATE drafts ... SET body = '<p>' || … || '</p>'` (HTML-escape `&`/`<`/`>` then `nl2br`) that only touches non-blank rows not already looking like HTML, mirrored byte-for-byte between the Alembic file and the runner so the two stay in lockstep. Migration **0036** is mixed (a DDL `ALTER`/`CREATE INDEX` for the `email_content` sliding-TTL column **plus** a `TRUNCATE` that invalidates HTML cached before the body+attachments read was unified) — so the runner must both extend the inline `CREATE TABLE email_content` (the `last_accessed_at` column, for fresh bootstraps) AND append the idempotent `ALTER`/`CREATE INDEX`/`TRUNCATE` after the 0035 block, in addition to advancing the stamp. Migration **0038** (`add_archive_box_value`, `down_revision = 0037` the generalize-`users` migration) adds `ARCHIVE` to BOTH the `box` and the `previous_box` CHECK constraints of `email_metadata`, and its runner mirror touches **two** sites — the inline `CREATE TABLE email_metadata` carries the final CHECK form (fresh bootstraps) AND a trailing `DROP CONSTRAINT IF EXISTS … ADD CONSTRAINT …` pair for both checks covers incremental upgrades, followed by the stamp. Forgetting either check (or the `previous_box` half) re-introduces the "trash an archived message → CHECK violation" 500. Migration **0039** (`add_signature_html_to_accounts`, `down_revision = 0038_add_archive_box_value`) is the current head: unlike 0036/0038 it is a plain additive `ADD COLUMN` (`ALTER TABLE accounts ADD COLUMN IF NOT EXISTS signature_html TEXT DEFAULT NULL`), so it only appends the `ALTER` and advances the stamp, and deliberately does **not** touch the inline `CREATE TABLE accounts` — exactly like `email_address` (0009). Extending the inline `CREATE TABLE` is required only for the 0036-style case (a column fresh bootstraps need for a non-additive reason), never as a blanket rule for every new column.

### Migration 0010 test seed is gated — keep the gate in lockstep across BOTH sites

The seed of migration 0010 (phantom user `inventadoParaEndpointGet` + exactly 100 sample emails — 50 per provider: 30 `ALL_MAIL` + 10 `SENT` + 4 `TRASH` + 6 `SPAM`, identical for the Gmail account `bbbbbbbb-bbbb-4000-a000-bbbbbbbbb001` and the Outlook account `bbbbbbbb-bbbb-4000-a000-bbbbbbbbb002`) is **opt-out / default-ON**, gated by `DB_SEED_TEST_DATA` and read through `settings.is_test_data_seed_enabled()` (`settings.py` is the only module allowed to touch `os.environ`). The gate is duplicated on purpose and the two halves MUST stay in lockstep:

1. The Alembic path — `0010.upgrade()` early-returns when the flag is off. Alembic still seals the revision (an empty `upgrade()` is a successful upgrade), so the schema is built and `0011+` apply; only the rows are skipped.
2. The runner fallback — the seed lives in `_SEED_STATEMENTS` (extracted out of `_DDL_STATEMENTS`, which stays pure schema + `alembic_version` stamps); `_apply_statements` runs it only when the flag is on.

Gating one site without the other is a silent regression a reviewer must catch. The flag is read **at apply time, via a deferred import inside the function** (not at module import / list construction), because the env may not be loaded when these modules are imported. `_SEED_STATEMENTS` is applied **after** all of `_DDL_STATEMENTS`: its `INSERT`s list only pre-0010 columns, and every later-added column carries a `DEFAULT`, so the trailing order produces the same final state. `downgrade()` is intentionally **not** gated — its `DELETE`s are idempotent, and skipping them when the flag is off would strand rows if the flag flipped between upgrade and downgrade.

**Cross-function dependency:** the fail-fast on an invalid (non-bool) `DB_SEED_TEST_DATA` only holds because `ensure_schema_at_head` carries an `except DatabaseError: raise` guard **before** its generic `except Exception`. `is_test_data_seed_enabled()` raises `SettingsError` (a `DatabaseError` subclass) from inside `_apply_statements`; without the guard the generic catch would re-wrap it as `MigrationError`, burying the fail-fast signal this gate promises. Removing the guard "because nothing in the SQL raises `DatabaseError`" misses that the `settings` call inside the `try` does.
