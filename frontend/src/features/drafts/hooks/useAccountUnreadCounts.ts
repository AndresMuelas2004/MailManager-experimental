...
// Duplicate of features/emails/hooks/useAccountUnreadCounts: the cross-feature
// import rule (features/CLAUDE.md §6) forbids importing the emails hook from the
// drafts feature. The query key is byte-for-byte identical (see comment below),
// so both copies still share ONE TanStack Query cache entry per (mailbox, box).
//
// Per-account unread counts for the account tabs (Bandeja/Spam). Fetches the
// mailbox-wide counts and extracts this account from the breakdown.
