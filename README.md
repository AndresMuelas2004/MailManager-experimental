...
- Custom folders + organisation rules: user-level folders that sync to the provider as Gmail user-labels / Outlook categories (multi-membership, orthogonal to the box — assigning a folder never moves the email, and no new OAuth scope is used), materialised per account on first use (adopting an existing same-name label/category). An internal rule engine (exact sender and/or subject-substring → assign a folder) classifies new mail at sync time and can be applied retroactively to already-synced mail in the background. Folder membership is reconciled from provider truth on every sync (like favourites). Renaming/deleting a folder is reflected at the provider (Gmail renames/deletes the label; Outlook re-tags each member since its category name is immutable) and deleting it removes the label from the account.
...
- Consent-gated web analytics (GA4) on the **public pages only** (`/`, `/privacy`, `/terms`, `/login`): nothing loads until the visitor accepts the banner, and the signed-in app is never measured. Optional per deploy (`VITE_GA_MEASUREMENT_ID`); frontend-only, no backend surface.
...
| `VITE_GA_MEASUREMENT_ID` | No | GA4 measurement id (`G-XXXXXXXXXX`) for public-pages web analytics. **Build-time** (baked into the bundle). Empty (default) disables analytics outright: no consent banner, no `gtag.js` request, no cookies. When set, the GA property's enhanced measurement must keep "page changes based on browser history events" OFF — see `docs/limits/analitica-web.md`. |
...
Folders (custom organisation labels — Gmail user-labels / Outlook categories, multi-membership, orthogonal to the box, no new OAuth scope):

- `GET /folders` — List the caller's folders.
- `POST /folders` — Create a folder from `{ name, color? }` (name is case-insensitively unique per user — 409 `folder_name_conflict` on collision). Does NOT materialise at the provider yet — that happens lazily on the first assignment.
- `GET /folders/{folder_id}` — Fetch one (404 `folder_not_found` for a foreign / missing id).
- `PATCH /folders/{folder_id}` — Rename and/or recolour. A rename reflects to the provider (Gmail patches the label; Outlook re-tags each member); a recolour is local-only.
- `DELETE /folders/{folder_id}` — Delete a folder (Provider-First best-effort: Gmail deletes the label, Outlook removes the category from every member), then cascades the local rows (`folder_account_links`, `email_folder_members`, and the folder's `rules` / `rule_apply_jobs`). Never deletes the emails.
- `GET /folders/{folder_id}/emails` — List the folder's members across all of the user's accounts. Same paginated envelope as `GET /emails`, deduplicated + grouped by thread; optional `q` (same lupa search as the regular listing), `limit` (default 50, max 500), `offset`. Membership is orthogonal to box (members show in every real box except the internal `DELETED`; a positive `in:` narrows it).
- `POST /mailboxes/{mailbox_id}/accounts/{account_id}/emails/{provider_message_id}/folders` — Assign a folder to a single email (Provider-First; body `{ folder_id }`). Materialises the folder in that account lazily (adopting an existing same-name label/category) then applies the label/category. Returns `{ folders: [{ folder_id, name, color }] }` (the email's folders after the op). Does NOT move/archive the email.
- `DELETE /mailboxes/{mailbox_id}/accounts/{account_id}/emails/{provider_message_id}/folders/{folder_id}` — Remove a folder from a single email (Provider-First). Same response shape.

Rules (internal classification engine — no native Gmail Filters / Outlook messageRules):

- `GET /rules` — List the caller's rules.
- `POST /rules` — Create a rule from `{ name?, match_from_email?, match_subject_contains?, target_folder_id, is_enabled?=true, apply_to_existing?=false }`. At least one condition is required (`match_from_email` exact, normalised to lower-case; and/or `match_subject_contains` substring — combined with AND); the action is fixed: assign the matched message to `target_folder_id` (must belong to the user — 404 otherwise). New mail is classified automatically at sync time. `apply_to_existing=true` enqueues the background "apply to existing" job.
- `GET /rules/{rule_id}` — Fetch one (404 `rule_not_found`).
- `PATCH /rules/{rule_id}` — Update fields. The at-least-one-condition invariant is re-checked against the merged rule — clearing both conditions returns 422 `rule_validation_error`.
- `DELETE /rules/{rule_id}` — Delete a rule.
- `POST /rules/{rule_id}/apply` — Trigger (or restart) the "apply to existing" job that scans every already-synced message matching the rule and assigns the target folder, in the background worker. Returns the job status.
- `GET /rules/{rule_id}/apply-status` — `{ status, processed_count, active }` where `status ∈ none|pending|running|completed|failed` (`none` = never applied) and `active` is true while pending/running. Local-only, intended for frequent polling.
...
- `folder_operation_error` — 500
- `rule_not_found` — 404
- `rule_validation_error` — 422
- `rule_operation_error` — 500
- `rule_apply_status_error` — 500
